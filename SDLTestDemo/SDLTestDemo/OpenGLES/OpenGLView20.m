...
- (void)displayYUV420pData:(void *)data width:(NSInteger)w height:(NSInteger)h
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        @synchronized(self)
        {
            if (w != self->_videoW || h != self->_videoH)
            {
                [self setVideoSize:w height:h];
            }
            [EAGLContext setCurrentContext:_glContext];
            
            glBindTexture(GL_TEXTURE_2D, self->_textureYUV[TEXY]);
            
            //glTexSubImage2D (GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, const GLvoid* pixels);
            
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w, h, GL_RED_EXT, GL_UNSIGNED_BYTE, self->_pYuvData);
            //[self debugGlError];
            glBindTexture(GL_TEXTURE_2D, self->_textureYUV[TEXU]);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w/2, h/2, GL_RED_EXT, GL_UNSIGNED_BYTE, self->_pYuvData + w * h);
            
            // [self debugGlError];
            
            glBindTexture(GL_TEXTURE_2D, self->_textureYUV[TEXV]);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w/2, h/2, GL_RED_EXT, GL_UNSIGNED_BYTE, self->_pYuvData + w * h * 5 / 4);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (nowtime.tv_sec != self->_time.tv_sec)
        {
//            printf("视频 %d 帧率:   %d\n", self.tag, _frameRate);
            memcpy(&self->_time, &nowtime, sizeof(struct timeval));
            self->_frameRate = 1;
        }
        else
        {
